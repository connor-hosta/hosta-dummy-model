...
class Model_Inference:
    ...
    def refinement(self, polygon):
        ...
        if cnts:
            epsilon = 0.0015 * cv2.arcLength(np.float32(cnts[0]), True)
    ...
    def get_inference(self, outputs, flag):
        ...
        if outputs != None:
            ...
            for i, pred_id in enumerate(pred_ids):
                ...
                objects.append({
                    ...
                    'polygon': self.refinement(segmentation[i]),
                    'bounding_box': list(map(int, bboxes[i].tolist())),
                    'measurement': None,
