...
def read_image(file_name, format=None):
    ...
    bucket_name = 'detection-real-world'
    s3Client = boto3.client('s3', aws_access_key_id=ACCESS_ID, aws_secret_access_key=ACCESS_KEY)
    print("file name heree", file_name)

    # Read image bytes with s3 client
    data = s3Client.get_object(Bucket=bucket_name, Key=file_name)
    body = data['Body'].read()
    img_bytes = io.BytesIO(body).read()
